...
use crate::services::result_services::{cipher_default, decipher_default};
use crate::structs::pay_code::{
    client::PayCodeClientRespBody,
    server::{CipheredReq, CipheredResp, Req, Resp},
};
...
pub fn resolve(ecard_id: String, acc_tr_type: u8) -> Result<PayCodeClientRespBody, ResolveError> {
    ...
    let res = client
        ...
        .json(&CipheredReq::new(&encode(cipher_default(
            serde_json::to_string(&Req::new(
    ...
    let res_handler = serde_json::from_str(&res.text().unwrap())
        .map_err(|_e| ResolveError::UpstreamRespUnreadable)?;
    let handled_res: CipheredResp = res_handler;
    match handled_res.info().is_none() {
        true => Err(ResolveError::IncorrectParam),
        _ => {
            let decoded = decode(handled_res.info().as_ref().unwrap())
                .map_err(|_e| ResolveError::DecodeIntoHexError)?;
            let deciphered =
                decipher_default(&decoded).map_err(|_e| ResolveError::DecipherError)?;
            let final_res = serde_json::from_str(&deciphered)
                .map_err(|_e| ResolveError::DecodeIntoUTF8Error)?;
            let final_res_handled: Resp = final_res;
            info!(
                "request success with ecarid: {} account type: {}",
                String::from(&ecard_id),
                acc_tr_type
            );
